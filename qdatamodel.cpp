...
//    auto bquery = query.exec("select name, url from background_info order by access_time desc limit 0, 10; ");
    auto bquery = query.exec("select name, url from background_info; ");
    if(bquery){
...
void QDataModel::onRemove(const QString& file)
{
    auto sql = QString("delete from background_info where url='%1';").arg(file);
//    qDebug() << sql;
    QSqlQuery query;
    auto bquery = query.exec(sql);
    if(!bquery) {
...
void QDataModel::reportError(const QSqlQuery & query)
{
    auto error = query.lastError();
    QMessageBox::warning(nullptr, tr("warning"), error.text() + "("+ error.nativeErrorCode() + ")");
    qDebug() << error.text() + "("+ error.nativeErrorCode() + ")";
}
